...
#pragma endregion
		...
		#pragma region OpenCL platform setup, device call, kernel build, etc.
			...
			err = clGetDeviceIDs(platformIds[1], CL_DEVICE_TYPE_CPU, 1, &device, NULL);
			if(err != CL_SUCCESS){
			...
			clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(pformvendor), &pformvendor, NULL);
			printf("%s\n",pformvendor);
			system("pause");
			...
			const char* fileName = "winfloptmem.cl";
