...
int main (int argc, const char * argv[]){
    ...
    //float** data = new float*[(maxSize-minSize)/100];
   // for (int i = 0; i < 21; ++i)
     //   data[i] = new float[21];
    ...
    for(int i = minSize; i < maxSize; i+=100){
        ...
        for(int j = minIters; j < maxIters; j*=2){
            ...
            while (sw.getTime() < maxTime){
                ...
                program_handle = fopen("flopfmatmem.cl", "r");
                if(!program_handle){
	    ...
	    std::cout << "Time: " << sw.getTime() << std::endl;
            ...
            float mflops = 2*n1f*n2f*countf*iters*1.0e-09/sw.getTime();
