...
//#define RAND_MAX 10000 for windows
...
int main (int argc, const char * argv[])
{
  ...
  for(int i =0; i<n1; i++) {
    for(int j=0; j<n2; j++) {
      printf("h_r=%f h_r1=%f\n", h_r[i][j], h_r1[i+j*n1]);//yindex + xindex*ysize
    }
  }
*/
  ...
  if(programType == 1) {
    ...
    size_t mapped_n1 = n2 / 2 ; //Note the switch here. I am not sure why the divide by 2 or the switch.
    ...
    //Writing out the computed values to files for plotting
    for(int i = 0; i < n1; ++i) {
      for(int j = 0; j < n2; ++j) {
        inputData << h_r[i][j] << "\t";
      }
