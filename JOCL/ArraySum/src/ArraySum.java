/*
 * File Name: "ArraySum.java"
 * Author: Joseph Jennings
 * Date: August 07, 2013
 * Description: This code sums two 2D arrays in parallel using JOCL.
 *
*/
...
public class ArraySum {
	...
	/*
	 * This is the code that gets sent to and executed on the GPU in order
	 * to sum the arrays in parallel. Each argument is stored in a buffer
	 * on the global memory of the GPU and then using the get_global_id
	 * function the arrays can be summed.
	 */
	...
	public static void main(String args[]){
		...
		//Declaring the x,y and z arrays of floats.
	   ...
	   //Compiles the program and stores the compiled code into the argument "program"
		...
		d_xx = clCreateBuffer(context, CL_MEM_READ_WRITE, Sizeof.cl_float*n, null, errM);
		if(errM[0] < 0){
		...
		for(int i = 0; i < n; i++){
			...
			err = clEnqueueWriteBuffer(queue, d_yy, CL_TRUE, 0, Sizeof.cl_float*n, ph_yy.to(h_yy[i]), 0, null, null);
			if(err < 0){
			...
			if(err < 0){
				System.out.println("Error: Could not set kernel argument h_yy");
				System.exit(1);	
			}
