...
public class LocalDiffusionKernel {
  ...
  public enum Stencil {
    ...
    D22,
    /** 
     * A 2x2 stencil implemented using JOCL
     * The 2D version has 4 non-zero coefficients.
     * The 3D version has 8 non-zero coefficients.
     * This stencil is the default. 
     */
  ...
  private CLUtil clInit = new CLUtil(sourceStr, "soSmoothingNew");
    ...
    Tensors2 d, float c, float[][] s, float[][] x, float[][] y) 
  {
    ...
    for (int i2=1; i2<n2; ++i2) {
      ...
      for (int i1=1,m1=0; i1<n1; ++i1,++m1) {
        d.getTensor(i1,i2,di); // how should I get the tensors to the GPU???
...
//		clReleaseContext(clInit.context);
  ...
  private void apply22MyCPU(Tensors2 d, float c, float[][] sc, float[][] r, float[][] s)
  {
	  int n1 = r.length;
	  int n2 = r[0].length;
	  ...
	  float[] dia = new float[n1*n2];
	  float[] dib = new float[n1*n2];
	  float[] dic = new float[n1*n2];
	  ...
	  for(int i2 = 1; i2 < n1; ++i2)
		{
			for(int i1 = 1; i1 < n2; ++i1)
			{
				...
				float e11 = c * dia[i2*n2 + i1];
				float e12 = c * dib[i2*n2 + i1]; 
				float e22 = c * dic[i2*n2 + i1];
				float r00 = r[i2  ][i1  ];
				float r01 = r[i2  ][i1-1];
				float r10 = r[i2-1][i1  ];
				float r11 = r[i2-1][i1-1];
				float rs = 0.25f*(r00+r01+r10+r11);
				float ra = r00-r11;
				float rb = r01-r10;
				float r1 = ra-rb;
				float r2 = ra+rb;
				float s1 = e11*r1+e12*r2;
				float s2 = e12*r1+e22*r2;
				float sa = s1+s2;
				float sb = s1-s2;
				s[i2  ][i1  ] += sa;
				s[i2  ][i1-1] -= sb;
