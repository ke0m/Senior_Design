...
endnum = 400
step = 20
cpudata = zerofloat(endnum/step - 1)
gpudata = zerofloat(endnum/step - 1)
alphas = zerofloat(endnum/step - 1)
j = 0

for i in xrange(20,400,20):
    sw.restart()
    while (sw.time() < maxTime):
        smooth_cpu.apply(d04, i, iimg, simgc)
    ...
    while(sw.time() < maxTime):       
        smooth_gpu.applyGPU(d04, i, iimg, simgg)
